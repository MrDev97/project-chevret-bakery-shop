"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.UserValidatorService = void 0;
const common_1 = require("@nestjs/common");
const user_require_unique_email_exception_1 = require("./exception/user-require-unique-email-exception");
const users_repository_1 = require("./db/users.repository");
let UserValidatorService = class UserValidatorService {
    constructor(userRepository) {
        this.userRepository = userRepository;
    }
    async validateUniqueEmail(email) {
        const usr = await this.userRepository.getUserByEmail(email);
        if (usr) {
            throw new user_require_unique_email_exception_1.UserRequireUniqueEmailException();
        }
    }
};
UserValidatorService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [users_repository_1.UserRepository])
], UserValidatorService);
exports.UserValidatorService = UserValidatorService;
//# sourceMappingURL=user-validator.service.js.map